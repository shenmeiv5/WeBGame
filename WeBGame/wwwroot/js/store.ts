...
function 精选_初始化() {
    ...
    //获取控件
    let info = document.body.getElementsByClassName("info");
    for (let k = 0; k < info.length; k++) {
        let screenshot = info[k].getElementsByClassName("screenshots");
        for (let i = 0; i < screenshot.length; i++) {
            let value = screenshot[i].getElementsByTagName("div");
...
function 精选_当鼠标滑过时变更图片(eventHandler: number) {
    是否滚动 = false;
    {
        /*
...
function 精选_当鼠标离开时取消图片(eventHandler: number) {
    //let focusEle = document.getElementsByClassName("focus")[0].getElementsByClassName("main_capsule")[0].getElementsByClassName("screenshot");
